...
test("locality belongs to material placement, not higher model or agency identity", () => {
  const ollama = receipt(
    relation({
      engine: { implementation_ref: "engine:ollama", provider_ref: "provider:aikit/ollama" },
      placement: "local",
      binding: "workcell:mac/ollama-42",
    }),
  );
  const vllm = receipt(
    relation({
      engine: { implementation_ref: "engine:vllm", provider_ref: "provider:aikit/vllm" },
      placement: "remote",
      binding: "workcell:gpu-server/vllm-9",
    }),
  );
  ...
  assert.equal(
    ollama.model_relation.inference_surface.contract_ref,
    vllm.model_relation.inference_surface.contract_ref,
  );
...
test("Colibri remains an experimental engine/material intervention rather than a provider ontology", () => {
  const colibri = receipt(
    relation({
      engine: {
        implementation_ref: "engine:colibri@4ef9a992",
        facts: { technique: "storage-ram-vram-staging", experimental: true },
      },
      placement: "local",
      binding: "workcell:experiment/colibri-1",
    }),
  );
  colibri.experiment = {
    ...
    variables: [
      {
        name: "inference-materialisation-technique",
        value_ref: "engine:colibri@4ef9a992",
        facts: { comparison: "ollama-llamacpp-vllm" },
      },
    ],
  };
...
test("provider-native scalar facts are preserved without becoming canonical fields", () => {
  const input = relation({
    engine: {
      implementation_ref: "engine:llama.cpp",
      provider_ref: "provider:aikit/llama.cpp",
      facts: { context_size: 32768, flash_attention: true, native_mode: "llama-server" },
    },
    placement: "local",
    binding: "workcell:mac/llama-server-3",
  });
...
test("invalid placement and malformed access fail before a receipt can be accepted", () => {
  const badRelation = relation({
    engine: { implementation_ref: "engine:ollama" },
    placement: "provider-ontology",
    binding: "workcell:bad",
  });
