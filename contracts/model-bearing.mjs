...
/**
 * Validate a provider-neutral ModelRelation.
 *
 * Engine, materialisation and inference-surface details remain nested refs/facts.
 * This contract deliberately does not create constitutional InferenceEngine,
 * ModelMaterialisation or ModelSurface identities.
 */
...
/**
 * Validate access independently from the ModelRelation.
 *
 * Inference permission does not imply model-control permission, and neither
 * implies model-interior access.
 */
...
/**
 * Validate an ActuationReceipt which binds a model-bearing condition to an
 * Actuation without turning provider/material facts into Agent identity.
 */
export function validateActuationReceipt(input) {
  ...
  if (receipt.experiment != null) {
    ...
    stringArray(experiment.held_constant_refs, "ActuationReceipt.experiment.held_constant_refs", {
      optional: true,
    });
    if (experiment.variables != null) {
      if (!Array.isArray(experiment.variables)) {
        throw new TypeError("ActuationReceipt.experiment.variables must be an array");
      }
      for (const [index, variable] of experiment.variables.entries()) {
        ...
        if (typeof variable.name !== "string" || variable.name.trim() === "") {
          throw new TypeError(`ActuationReceipt.experiment.variables[${index}].name must be non-empty`);
        }
        ref(variable.value_ref, `ActuationReceipt.experiment.variables[${index}].value_ref`, {
          optional: true,
        });
