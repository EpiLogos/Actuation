...
**Reconciled 2026-09-09:** recovered from closed draft PR #3 head `97347cdcb82bca56430beb955b89cf9d9885b97f` as agent-authored research ground. Current main already carries `actuation.instantiation/v0` for model-bearing receipts and `actuation.model-usage/v1` for provider-supplied usage observations. The epistemic experiment adds graded access/provenance records locally; none of these contracts establishes provider control or model-interior availability by declaration alone.
...
1. **Actuation:** continue from the current instantiation receipt, which distinguishes model/variant, inference engine, model surface, material binding and inference/control access beneath Agent/Agency identity. The epistemic experiment separately declares model-interior access depth and requires real method/evidence refs before availability can be recorded.
